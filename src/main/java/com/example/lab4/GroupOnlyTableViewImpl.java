...
*/
public class GroupOnlyTableViewImpl implements IGroupTableView {
    ...
    public void createGroupView(TreeTableView<Student> startingTable) {
        ...
        TreeTableColumn<Student, String> treeTableColumn1 = new TreeTableColumn<>("Group name");
        treeTableColumn1.setCellValueFactory(new TreeItemPropertyValueFactory<>("name"));
        startingTable.getColumns().addAll(treeTableColumn1);
        startingTable.setColumnResizePolicy(TreeTableView.CONSTRAINED_RESIZE_POLICY);
    }

    /**
     * Updates the data in the provided TreeTableView based on the given list of groups.
     *
     * @param startingTable The TreeTableView to update.
     * @param groups        The list of groups containing the data to be displayed.
     */
    @Override
